...
class ResNet(nn.Module):
    ...
    def apply(self, pil):
        ...
        if (pil.size[0]>LIMIT_SIZE) and (pil.size[1]>LIMIT_SIZE):
            ...
            logit[0] = np.mean(np.asarray(list_logit)[:,0] * np.asarray(list_weight)) / np.mean(list_weight)
            logit[1] = np.mean(np.asarray(list_logit)[:,1] * np.asarray(list_weight)) / np.mean(list_weight)
            logit = torch.from_numpy(logit)
        else:
            with torch.no_grad():
                logit = torch.squeeze(self(self.transform(pil).to(device)[None,:,:,:])).cpu()#.numpy()
                
        tbs = nn.Softmax(dim=0)(logit)
        score = tbs[1]
        
        return score


def resnet50ft(device, num_classes=2):
